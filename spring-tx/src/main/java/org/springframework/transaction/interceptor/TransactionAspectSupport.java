...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, Class<?> targetClass, final InvocationCallback invocation)
		...
		// 获取 beanFactory 中 transactionMessage
		...
		if (txAttr == null || !(tm instanceof CallbackPreferringPlatformTransactionManager)) {
			...
			try {
				...
				// This will normaly result in a target object being invoked.
				// 执行被增强的方法
	...
	protected TransactionInfo createTransactionIfNecessary(
		...
		// 如果没有名称指定则使用方法唯一标示、并使用 DelegatingTransactionAttribute 封装 txAttr
		...
		if (txAttr != null) {
			if (tm != null) {
				// 获取 Transaction、AbstractPlatformTransactionManager 类
		...
		// 根据指定属性与 status 准备一个 TransactionInfo
	...
	protected void commitTransactionAfterReturning(TransactionInfo txInfo) {
		if (txInfo != null && txInfo.hasTransaction()) {
			...
			// AbstractPlatformTransactionManager.commit 函数
	...
	protected void completeTransactionAfterThrowing(TransactionInfo txInfo, Throwable ex) {
		...
		if (txInfo != null && txInfo.hasTransaction()) {
			...
			// 这里判断是否回滚默认的依据是抛出的异常是否是 RuntimeException 或者 Error 的类型 TODO 重点方法
			if (txInfo.transactionAttribute.rollbackOn(ex)) {
				try {
					// 根据 TransactionStatus 信息进行回滚处理  TODO 重点方法、AbstractPlatformTransactionManager 类
